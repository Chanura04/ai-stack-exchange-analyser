...
posts_asset = Asset("s3://data-platform-post-users/raw/Posts.xml")
users_asset = Asset("s3://data-platform-post-users/raw/Users.xml")
...
def produce_data_assets():
    ...
    # Load the file to S3 (with debug logging and pause)
    s3_hook = S3Hook(aws_conn_id="aws_conn")
    posts_file = os.path.join(extract_path, "Posts.xml")
    users_file = os.path.join(extract_path, "Users.xml")
    try:
        logging.info("Uploading %s to s3://data-platform-post-users/raw/Posts.xml", posts_file)
        s3_hook.load_file(
            filename=posts_file,
            key="raw/Posts.xml",
            bucket_name="data-platform-post-users",
            replace=True
        )
        logging.info("Uploading %s to s3://data-platform-post-users/raw/Users.xml", users_file)
        s3_hook.load_file(
            filename=users_file,
            key="raw/Users.xml",
            bucket_name="data-platform-post-users",
        ...
        logging.info("S3 uploads completed successfully")
    except Exception:
        logging.exception("S3 upload failed")
        raise
    finally:
        logging.info("Debug pause: sleeping 30s to keep pod alive for inspection (remove this in production)")
        time.sleep(30)
